function [AS_IHPC,AS_IHPC_ICPC,AS,IHPC,ICPC] = MaximumVoicedFrequencyEstimation_nopp(wave,Fs,f0,t)
...
%  f0             : vector containing the F0 estimates (0 values are
%                    provided in unvoiced parts).
%  t            : [samples] Analysis instants of the f0 values. These will
...
%
% Example
%  Please see the Demo_MVF.m example file.
%
% References
...
for n=1:length(t)
    ...
    if f0(n)>0
		...
		% is the vector form faster?
		% p1=gaussmf_new_vec(Candidates,[Std1 Mu1]);
		% p2=gaussmf_new_vec(Candidates,[Std2 Mu2]);
		% Probas1=p1./(p1+p2);
		...
		% is the vector form faster?
		% p1=gaussmf_new_vec(Candidates2,[Std1 Mu1]);
		% p2=gaussmf_new_vec(Candidates2,[Std2 Mu2]);
		% Probas2=(1/Alpha)*p1./(p1+p2);
...
function y_vec = gaussmf_new_vec(x_vec, params)
...
sigma = params(1); c = params(2);
y_vec = (1/(sqrt(2*pi)*sigma))*exp(-(x_vec - c).^2./(2*sigma^2));